...
build a larger universe.  Note this is a live O(N^2) N-body simulator: a few
hundred bodies run comfortably; many thousands will not run in real time.
"""
import argparse
import json
import os
...
def cap_nearest(bodies, max_systems):
    ...
    keeps everything. This is what makes the result runnable: the sim is O(N^2),
    so the full catalogs (~16k bodies) must be trimmed to a nearby slice."""
    if max_systems <= 0:
        return bodies
    by_name = {b.get("name", ""): b for b in bodies}
...
def main():
    ...
    ap.add_argument("--max-systems", type=int, default=60,
                    help="keep the Solar System + this many nearest star systems "
                         "(0 = everything; the sim is O(N^2), so huge values "
